...
In functional programming, as the name suggests, functions are the building blocks of programs. At the top, we find $main$ that, like any other function, takes an input and returns an output. Usually, it comprises other functions that are, in turn, made up of even more functions. Eventually, at the very bottom of the chain, we find functions that are language primitives. \ref{eq:id} and \ref{eq:map} show examples of such in Haskell, the programming language we use throughout this paper~\cite{hughes1989functional, hu2015functional, website-haskell-prelude}.
...
This paper explores whether Bird’s approach can be applied to Str8ts, a puzzle similar to Sudoku. See Figure~\ref{fig:grid-comparison-sudoku-str8ts} for a side by side comparison. For the implementation, we chose Haskell as our programming language, GHCup as tool management and Stack as dependency management~\cite{website-haskell, website-ghcup, website-stack}.
