...
We can apply equational reasoning for various purposes. Let’s take a look at \ref{eq:fibonacci-2}. Here, we use it to prove that $fibonacci~2$ equals $1$. This example also gives a glimpse into the notation we use throughout the thesis. In general, we write functions as we would in Haskell. For better readability, we replace logical operators with their mathematical counterparts. E.g, $\&\&$ with $\land$. We also write $\neq$, instead of $/=$. On top, the text in curly brackets between transformation steps provides information on the applied justifying facts and rules, later called theorems.
