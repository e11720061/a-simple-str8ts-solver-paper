...
We start by agreeing on a Haskell representation of the puzzle. We introduce generic types to build upon.
...
The puzzle is played on an $  n\times n$ grid. We choose $n=9$ as it is the most common. But, if necessary, 9 can be replaced by any positive integer. Elements inside the grid are called squares. A $9 \times 9$ grid, as a consequence, contains a total of 81 squares.
...
Squares with a value of 0 are empty. A complete grid contains no empty white squares. However, it might contain empty black squares. In the following, we will refer to empty, white squares as blank squares. We introduce the function $blank$ to determine whether a square is blank.
...
The following is an example of an incomplete $9 \times 9$ grid. For readability, we abbreviated the constructors. We simply write $S$ for $Square$, $W$ for $White$, and $B$ for $Black$. For example, $S~W~3$ abbreviates $Square~White~3$.
...
Last but not least, we have to define the functions $rows$, $columns$, and $compartments$. We aim for wholemeal programming, a term coined by Geraint Jones~\cite{hinze2009functional}. The goal of this paradigm is to view data types holistically and avoid succumbing to Indexitis. In our representation, we store a matrix as an array of rows. Hence, $rows$ is equal to the $id$ function.
...
To make it easier for us, we use the module Data.List~\cite{website-haskell-list}. To access the columns, we can use the function $Data.List.transpose$.
...
In contrast to \(rows\) and \(columns\), the function $compartments$ is not polymorphic. It takes a row or column of type \([Square]\) as input and groups it into a list of white and black compartments. It uses \(Data.List.groupBy\) and we can revert it by applying \(concat\).
